...
bot = telebot.TeleBot("TOKEN DO BOT AQUI")
index_function = 0


@bot.message_handler(commands=['start'])
def send_welcome(message):
    bot.reply_to(message,"Bem-vindo(a) ao atendimento virtual do Brasil Participativo. Para começar, diga-nos o seu nome")
...
@bot.message_handler(func=lambda message: message.text.isdigit() and (1 <= int(message.text) <= 7) and index_function == 0)
def handle_messages(message):
    ...
    match mensagem_usuario:
        case "1":
            index_function = 1
            bot.send_message(id_chat,"Brasil Participativo é a nova plataforma de participação social do governo federal,"
...
@bot.message_handler(func=lambda message: message.text.isdigit() and (1 <= int(message.text) <= 7) and index_function == 1)
def handle_messages(message):
    global index_function
    id_chat = message.chat.id
    match message.text:
        case "1":
            bot.send_message(id_chat,"Agradecemos o seu contato e continuamos à disposição! Para acompanhar todas as atividades realizadas pelo Brasil Participativo, acesse https://brasilparticipativo.presidencia.gov.br/")
        case "2":
            index_function = 2
            bot.send_message(id_chat, "Digite 9 - Retornar ao menu anterior ?\nCaso queira encerrar o atendimento, digite “sair”")
