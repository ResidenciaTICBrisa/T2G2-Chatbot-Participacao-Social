    ...
    match mensagem_usuario:
        ...
        case "4":
            ...
            bot.reply_to(message,
                         "Para aderir à plataforma ou tirar dúvidas sobre os processos participativos, por favor, entre em contato pelo e-mail: participacaodigital@presidencia.gov.br\n\nAcesse os nossos cursos de capacitação na plataforma EVG (quando estiverem disponibilizados)\n")
            bot.reply_to(message, "Sua demanda foi atendida? \n1 - Sim \n2 - Não")
        case "5":
            sm_dict[id_chat].setstate(1)
        ...
        case "6":
            sm_dict[id_chat].setstate(1)
            bot.reply_to(message,
                         "Confira as principais perguntas sobre os serviços relacionados ao Brasil Participativo (inserir link, da plataforma, de perguntas frequentes)\n")
            bot.reply_to(message, "Sua demanda foi atendida? \n1 - Sim \n2 - Não")
        case "7":
            sm_dict[id_chat].setstate(1)
            bot.reply_to(message,
                         "Para assuntos relacionados à plataforma Brasil Participativo, por favor, entre em contato pelo e-mail:\nparticipacaodigital@presidencia.gov.br\nPara outros assuntos, registre a sua manifestação na Plataforma Integrada de Ouvidoria e Acesso à Informação (Fala.BR) https://falabr.cgu.gov.br\n")
            bot.reply_to(message, "Sua demanda foi atendida? \n1 - Sim \n2 - Não")
...
@bot.message_handler(
    func=lambda message: message.text.isdigit() and (1 <= int(message.text) <= 6) and compare(message.chat.id, 3))
def handle_messages(message):
    global sm_dict
    id_chat = message.chat.id
    match message.text:
...
# Plano clima
@bot.message_handler(
    func=lambda message: message.text.isdigit() and (1 <= int(message.text) <= 4) and compare(message.chat.id, 4))
def handle_messages(message):
    global sm_dict
    id_chat = message.chat.id
    match message.text:
        case "1":
            sm_dict[id_chat].setstate(1)
            bot.send_message(id_chat,
                             "O Plano Clima é uma estratégia governamental adotada pelo Brasil para lidar com as "
        ...
        case "4":
            sm_dict[id_chat].setstate(1)
            bot.send_message(id_chat,
                             "O Plano Clima foi instituído em 2009 para consolidar a execução da Política Nacional"
                             " de Mudança do Clima.\n"
...
# Conferências
@bot.message_handler(
    func=lambda message: message.text.isdigit() and (1 <= int(message.text) <= 4) and compare(message.chat.id, 5))
def handle_messages(message: object):
    global sm_dict
    id_chat = message.chat.id
    match message.text:
...
# Consultas Públicas
@bot.message_handler(
    func=lambda message: message.text.isdigit() and (1 <= int(message.text) <= 4) and compare(message.chat.id, 6))
def handle_messages(message: object):
    global sm_dict
    id_chat = message.chat.id
    match message.text:
        case "1":
            sm_dict[id_chat].setstate(1)
            bot.send_message(id_chat,
                             "É um mecanismo de participação social, de caráter consultivo, realizado com prazo "
...
# Enquetes
@bot.message_handler(
    func=lambda message: message.text.isdigit() and (1 <= int(message.text) <= 4) and compare(message.chat.id, 7))
def handle_messages(message: object):
    global sm_dict
    id_chat = message.chat.id
    match message.text:
        case "1":
            sm_dict[id_chat].setstate(1)
            bot.send_message(id_chat,
                             "As Enquetes visam a dar maior publicidade e transparência ao trabalho de elaboração"
...
# Audiências Públicas
@bot.message_handler(
    func=lambda message: message.text.isdigit() and (1 <= int(message.text) <= 4) and compare(message.chat.id, 8))
def handle_messages(message: object):
    global sm_dict
    id_chat = message.chat.id
    match message.text:
...
# Colegiados
@bot.message_handler(
    func=lambda message: message.text.isdigit() and (1 <= int(message.text) <= 4) and compare(message.chat.id, 9))
def handle_messages(message: object):
    global sm_dict
    id_chat = message.chat.id
    match message.text:
        case "1":
            sm_dict[id_chat].setstate(1)
            bot.send_message(id_chat,
                             "Órgãos Colegiados referem-se a um corpo consultivo e/ou deliberativo que tem como"
